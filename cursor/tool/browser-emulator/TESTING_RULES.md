...
1. If `output/review-mode.local.json` is missing, **ask the human**:
   ...
   - This computer (local-open: proxy + form, no Docker tunnel)
...
4. If they want feedback or testing on **another device**, hands-free is required. Do not use CDP `holdForUserAnswer` for that. For same-machine overlay feedback without a tunnel, use **local-open** (`hands-free --localOpen true`).
